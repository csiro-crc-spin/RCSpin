...
test.outcome<-tail(alex$clinical_history$events,1)[[1]] #returns a list -- the first item of which is a Test
test.outcome$type
model<-DukesCrcSpinModel$new(iterations=99, num_subjects=5)
 model$NBCSP(alex)
Error in envRefInferField(x, what, getClass(class(x)), selfEnv) (from aa.R#1929) : 
  ‘NBCSPRecordSize’ is not a valid field or method name for reference class “PersonWithColon”
