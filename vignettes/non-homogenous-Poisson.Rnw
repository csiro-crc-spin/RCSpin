...
Writing $N(t)$ for the number of events by time $t$. A stochastic process is an inhomogeneous Poisson process for some small value $h$ if
...
Using equation \ref{equation:poisson_calculation} we can calculate the risk for a single year. As we are working in yearly increments
the integrated risk  $R$ will be very lose to the risk at the end point. Figure \ref{figure:compare_non-homogenous_Poisson_process_Poisson_distn.png}
shows the number of people initiating an adenoma each year, from ages 20 to 70. This is calculated by a
